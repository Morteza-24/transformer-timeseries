...
\begin{itemize}
  ...
  \item We analyze inductive-bias differences versus ordered sLSTM mixing (equivariance, global interactions, architectural simplification, and parallelism), not only forecasting accuracy, and report training throughput.
...
\begin{enumerate}
  ...
  Attention costs $O(C^2 d)$ per layer and may become expensive for very large $C$.
  Section~\ref{sec:throughput} reports training throughput under this tradeoff; we do not claim a general wall-clock advantage.
\end{enumerate}
...
All experiments were executed on an NVIDIA L4 GPU.
...
Table~\ref{tab:throughput} reports training throughput (iterations per second) for xLSTM-Mixer and the PE-free Transformixer default under the shared mixer recipe on an NVIDIA L4 GPU (batch size $32$, $L{=}H{=}96$).
These figures include the forward and backward pass together with data loading and logging; they are not isolated FLOPs, peak-memory, or inference-latency measurements.

On ECL ($C{=}321$), Transformixer trains at $4.67$ it/s versus $3.53$ it/s for xLSTM-Mixer ($\approx$32\% higher throughput).
On Traffic ($C{=}862$), the two are essentially tied ($1.33$ vs.\ $1.32$ it/s).
The pattern is consistent with token-parallel attention being advantageous at moderate $C$, while quadratic attention cost offsets that advantage at high $C$.
We therefore do not claim a general wall-clock superiority.

\begin{table}[t]
  \centering
  \caption{Training throughput (it/s; higher is faster) on an NVIDIA L4 GPU under the shared mixer recipe (batch size $32$, $L{=}H{=}96$). Transformixer is the PE-free default.}
...
Table~\ref{tab:throughput} matches that tradeoff: Transformixer is faster to train on ECL and matches xLSTM-Mixer on the high-$C$ Traffic benchmark.
Because the numbers are single-run training throughputs rather than FLOPs or inference latency, we do not claim general hardware superiority.
...
A fuller evaluation would include horizons $\{96,192,336,720\}$, additional datasets (e.g., ETT, Weather), multi-seed mean$\pm$std, ablations over mixer depth and head count, and FLOPs / peak-memory profiling.
...
On an NVIDIA L4 GPU, Transformixer trains faster on ECL and matches xLSTM-Mixer on Traffic, consistent with parallel mixing at moderate $C$ and quadratic attention cost at high $C$.
...
Experiments were run on NVIDIA L4 GPUs using cloud notebooks (Google Colab).
