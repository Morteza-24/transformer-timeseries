...
On Electricity and Traffic with lookback and horizon $96$, Transformixer is competitive with strong baselines: it improves upon xLSTM-Mixer on Traffic (including the best MAE among compared models) while remaining slightly behind on Electricity.
Our study is intentionally limited in breadth by available compute and time (two datasets, one horizon); nevertheless, the present results suggest that Transformixer is a promising direction, and future work can explore this idea more thoroughly across additional datasets, horizons, and training budgets.
\end{abstract}
...
\begin{itemize}
  ...
  \item We compare against five baselines on Electricity and Traffic ($L{=}H{=}96$) and discuss when the redesign helps, under clearly stated  limitations.
\end{itemize}
...
Transformixer's omission of positional encodings along the variate axis is a concrete realization of that principle: the mixer is permutation-equivariant by construction, in contrast to recurrent ordered mixing and to Transformer mixers that inject positional signals on the channel axis.
...
Crucially, we \emph{omit positional encodings} and use full (non-causal) attention.
...
Because the embedding, attention, feed-forward maps, and output projection are applied identically to every token and attention is a symmetric function of the token set, for any channel permutation matrix $P$,
\begin{equation}
...
Table~\ref{tab:main} and Figure~\ref{fig:bars} summarize the main comparison.
Transformixer is competitive overall and does not dominate every cell.

On \textbf{Traffic}, Transformixer improves upon its direct predecessor xLSTM-Mixer on both MSE ($0.4068$ vs.\ $0.4157$) and MAE ($0.2558$ vs.\ $0.2607$), achieving the best MAE among all compared models and the second-best MSE after iTransformer.
This is consistent with the hypothesis that direct, order-agnostic attention is particularly useful when many channels interact without a meaningful 1-D order.

On \textbf{ECL}, xLSTM-Mixer and iTransformer remain slightly ahead (MSE $0.1480$ / $0.1481$ versus $0.1517$ for Transformixer).
The gap is modest ($\approx 2.5\%$ relative MSE versus xLSTM-Mixer), suggesting that ordered sLSTM mixing with memory tokens remains a strong alternative at moderate $C$, or that Transformixer hyperparameters inherited from the xLSTM-Mixer setup are not fully optimal for ECL.
...
Relative to xLSTM-Mixer specifically, Transformixer trades a small regression on ECL for a clear gain on the higher-dimensional Traffic benchmark (Figure~\ref{fig:relative}).
...
\begin{figure}[t]
  ...
  \end{minipage}
  \caption{MSE (top) and MAE (bottom) on ECL and Traffic ($L{=}H{=}96$). Transformixer bars are outlined.}
  \label{fig:bars}
\end{figure}
...
\begin{figure}[t]
  ...
  \caption{Relative MSE change of Transformixer versus xLSTM-Mixer (negative is better for Transformixer).}
  \label{fig:relative}
\end{figure}

\subsection{Permutation behavior}
\label{sec:perm}
By construction, Transformixer is permutation-equivariant along channels: for a channel permutation matrix $P$, $f(XP)=f(X)P$.
Unit tests in our implementation confirm this identity up to numerical tolerance.
...
To probe the practical effect of this distinction, we load the Traffic-trained Transformixer and xLSTM-Mixer checkpoints and evaluate MSE/MAE under the identity channel order and $K{=}8$ independent random permutations of the variate axis (Table~\ref{tab:perm}).
...
The Traffic improvement aligns with the motivation in Section~\ref{sec:rationale}: high-$C$ forecasting benefits from direct pairwise mixing and set equivariance.
The permutation probe in Section~\ref{sec:perm} further shows that Transformixer remains stable under channel shuffles and consistently outperforms xLSTM-Mixer on that controlled evaluation, while the attention map illustrates structured cross-variate interactions.
On ECL, the slightly better xLSTM-Mixer result cautions against a universal claim that attention is a strictly superior variate mixer.
Recurrent mixing with memory tokens may still be competitive when $C$ is moderate, when training budgets are short, or when hyperparameters are tuned for the sLSTM design.
...
A fuller evaluation would include horizons $\{96,192,336,720\}$, additional datasets (e.g., ETT, Weather), multi-seed mean$\pm$std, controlled ablations (with vs.\ without positional encodings; number of layers/heads), and efficiency profiling.
...
The redesign targets a more natural inductive bias for unordered channels: permutation equivariance, direct multi-head interactions, and a simpler single-view architecture without memory tokens.
Empirically, Transformixer is competitive with strong baselines under a limited protocol, improving on xLSTM-Mixer for Traffic while remaining close on Electricity.
We view the contribution as an architectural study with transparent resource limits, and as evidence that set-structured attention is a promising drop-in alternative for joint variate mixing in mixer-style forecasters.
